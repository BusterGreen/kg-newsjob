...
function TakeOutVehicle(vehicleInfo)
    ...
    KGCore.Functions.TriggerCallback('KGCore:Server:SpawnVehicle', function(netId)
        ...
        TriggerServerEvent("kg-newsjob:server:addVehicleItems", KGCore.Functions.GetPlate(veh))
...
function MenuGarage()
    ...
    local Vehicles = Config.Vehicles[KGCore.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "kg-newsjob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function MenuHeliGarage()
    ...
    local Helicopters = Config.Helicopters[KGCore.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Helicopters) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "kg-newsjob:client:TakeOutHelicopters",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('kg-newsjob:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('kg-newsjob:client:TakeOutHelicopters', function(data)
    local vehicle = data.vehicle
